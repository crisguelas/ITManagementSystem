...
- **Units** — Academic and administrative organizational units managed from **Organization → Academic & Administrative Units** with standard unit CRUD management
...
- **Authentication** — NextAuth.js with hashed passwords, lightweight credentials sign-in throttling (email + IP failure tracking with short lockouts/backoff), and an absolute **12-hour** JWT session lifetime (users must sign in again after expiry)
